/*
 * This is a modified version of bbutil.c released by Research
 * In Motion Limited (Original license information below). 
 *
 * This file was modified by David Breakey (www.davidbreakey.ca).
 *
 * ----------
...
struct font_t {
    ...
    float pt;
    float advance[FONT_NUM_CHARS];
    float width[FONT_NUM_CHARS];
    float height[FONT_NUM_CHARS];
    float tex_x1[FONT_NUM_CHARS];
    float tex_x2[FONT_NUM_CHARS];
    float tex_y1[FONT_NUM_CHARS];
    float tex_y2[FONT_NUM_CHARS];
...
font_t* bbutil_load_font(const char* path, int point_size, int dpi) {
    ...
    //Let each glyph reside in 64x32 section of the font texture
    int segment_size_x = 0, segment_size_y = 0;
    int num_segments_x = 16;//this sets maximum glyph width indirectly
    int num_segments_y = 16;//this set maximum glyph height indirectly
    ...
    int font_tex_width = nextp2(num_segments_x * segment_size_x);//1024
    int font_tex_height = nextp2(num_segments_y * segment_size_y);//512
    ...
    for(c = 0; c < FONT_NUM_CHARS; c++) {
        ...
        div_t temp = div(c, num_segments_x);//240/16

        bitmap_offset_x = segment_size_x * temp.rem;//0
        bitmap_offset_y = segment_size_y * temp.quot;//15
...
//    }
    ...
    wchar_t * wcmsg = (wchar_t*)malloc(sizeof(wchar_t)*(msg_len+1));
    int wmsg_len = mbstowcs(wcmsg,msg,msg_len);
//    fprintf(stderr,"\n%s\n",msg);
//    for(i=0;i<wmsg_len;i++){
//    	fprintf(stderr,"/%4.4i",wcmsg[i]);
//    }
//    fprintf(stderr,"\nstr:%i mb:%i\n",msg_len,wmsg_len);

    vertices = (GLfloat*) malloc(sizeof(GLfloat) * 8 * wmsg_len);
    texture_coords = (GLfloat*) malloc(sizeof(GLfloat) * 8 * wmsg_len);
    indices = (GLshort*) malloc(sizeof(GLfloat) * 6 * wmsg_len);
    ...
    for(i = 0; i < wmsg_len; ++i) {
        ...
        texture_coords[8 * i + 0] = font->tex_x1[c];//239258
        texture_coords[8 * i + 1] = font->tex_y2[c];//718750
        texture_coords[8 * i + 2] = font->tex_x2[c];//244141
        texture_coords[8 * i + 3] = font->tex_y2[c];//718750
        texture_coords[8 * i + 4] = font->tex_x1[c];//239258
        texture_coords[8 * i + 5] = font->tex_y1[c];//708984
        texture_coords[8 * i + 6] = font->tex_x2[c];//244141
        texture_coords[8 * i + 7] = font->tex_y1[c];//708984
...
//	vertices[8 * 0 + 0] = 0 + 0 + 0;
//    vertices[8 * 0 + 1] = 0 + 0;
//    vertices[8 * 0 + 2] = vertices[8 * 0 + 0] + 512;
//    vertices[8 * 0 + 3] = vertices[8 * 0 + 1];
//    vertices[8 * 0 + 4] = vertices[8 * 0 + 0];
//    vertices[8 * 0 + 5] = vertices[8 * 0 + 1] + 1024;
//    vertices[8 * 0 + 6] = vertices[8 * 0 + 2];
//    vertices[8 * 0 + 7] = vertices[8 * 0 + 5];
////
//    texture_coords[8 * 0 + 0] = 0;//font->tex_x1[c];//239258
//    texture_coords[8 * 0 + 1] = 1;//font->tex_y2[c];//718750
//    texture_coords[8 * 0 + 2] = 1;//font->tex_x2[c];//244141
//    texture_coords[8 * 0 + 3] = 1;//font->tex_y2[c];//718750
//    texture_coords[8 * 0 + 4] = 0;//font->tex_x1[c];//239258
//    texture_coords[8 * 0 + 5] = 0;//font->tex_y1[c];//708984
//    texture_coords[8 * 0 + 6] = 1;//font->tex_x2[c];//244141
//    texture_coords[8 * 0 + 7] = 0;//font->tex_y1[c];//708984
//
...
//}
    ...
    glDrawElements(GL_TRIANGLES, 6 * wmsg_len, GL_UNSIGNED_SHORT, indices);
...
void bbutil_measure_text(font_t* font, const char* msg, float* width, float* height) {
    ...
    const int msg_len  =strlen(msg);
    wchar_t * wcmsg = (wchar_t*)malloc(sizeof(wchar_t)*(msg_len+1));
    const int wmsg_len = mbstowcs(wcmsg,msg,msg_len);
